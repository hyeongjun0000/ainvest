...
**AInvest**는 이러한 문제를 해결하기 위해 개발된 시스템입니다. 실시간 금융 데이터 수집 기술과 알고리즘을 결합하여, 사용자가 종목 티커와 시드 머니를 입력하는 것만으로 **현재 시장 상황, 기술적 지표 상태, AI 기반 매수/매도 신호 및 예상 목표가**를 한눈에 확인할 수 있도록 의사결정을 단순화해 줍니다.

## Key Features

*  **다중 시장 지원**: 한국(KOSPI, KOSDAQ) 및 미국(NASDAQ, NYSE) 주식 실시간 데이터 분석
*  **AI 매매 전략 **: LONG / SHORT / HODL 신호 제공 및 진입가, 목표가, 방어선(손절가) 자동 산출
*  **기술적 분석 자동화 **: 스토캐스틱, RSI, 거래량 배수, 이동평균선(MA), 신고가 대비 등락률 계산
* **리스크 매니지먼트 **: 기술적/정성적 요소를 점수화한 시장 위험도 지표 제공
*  **컨센서스 및 실적 추적**: 애널리스트 목표가 범위, 매매 의견 수집 및 다음 실적 발표일(D-Day) 분석
*  **하락장 대응 **: 시장 하락 추세 감지 및 숏(Short) 신호 발생 시 인버스 ETF 전략 가이드 제공
...
2.  **`StockAnalyzer`**: 기술 지표 계산, AI 매매 레벨 산출, 리스크 스코어링 담당
...
```
실행 후 터미널에 표시되는 Local URL (보통 http://localhost:8501)로 브라우저를 통해 접속합니다.

## How to Use

1. 시드 자금 입력: 좌측 사이드바 또는 상단 입력창에 투자에 사용할 시드 머니를 입력합니다. (추천 매수 수량 계산에 활용됩니다.)
2. 티커(Ticker) 검색: 분석하고자 하는 종목의 티커 심볼을 입력합니다. (예: AAPL - 미국장, 005930.KS - 코스피)
3. 분석하기: 버튼을 누르면 실시간 데이터를 바탕으로 AI 분석이 진행됩니다.
4. 결과 확인:
- 추세 흐름과 스토캐스틱 상태를 확인합니다.
- AI가 제안하는 신호(LONG/SHORT)와 진입가, 목표가를 확인하여 매매 계획을 세웁니다.
